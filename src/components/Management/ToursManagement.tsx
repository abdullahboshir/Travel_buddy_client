...
console.log('userrrrrrrrrrrrrrr', tripRes)
        ...
        <div key={tour.id} className="bg-gray-200 my-2">
          <div className="flex items-start justify-start">
            <div className="">
              ...
              <div className="flex gap-1 w-[350px]">
                {tour?.photos.map((photo: any, index: number) => (
                  ...
                  >
                    <Image layout="fill" objectFit="cover" className="" src={photo} alt="Avatar Tailwind CSS Component" />
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="bg-base-400 mt-4 ml-5 px-5 pt-5">
              <h1 className="text-3xl mb-2 font-semibold"> {tour.destination}</h1>
              <h1 className="text-2xl text-cyan-500">Let&apos;s enjoy the {tour.type} tour</h1>
              <h1 className="text-lg"> {tour.itinerary}</h1>
              <h1 className="text-lg mt-2 mb-2">
                <span className="text-cyan-500 font-semibold mt-5 flex flex-rows">Tour Activities:</span>{' '}
              ...
              <div className={`flex justify-end pb-5 ${seeMore[tour.id] ? 'py-4' : ''} ${tour.description.length < 280 && 'mt-[100px]'}`}>
                <Link href={`/dashboard/tours/update-trip/${tour.id}`} className="px-3 py-2 rounded-lg text-white bg-[#00c2ab] text-md hover:bg-[#083344] hover:font-bold hover:text-[16px] ease-in duration-100 transition">
                  Edit
                </Link>
