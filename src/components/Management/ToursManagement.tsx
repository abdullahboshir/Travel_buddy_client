    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        if (tripRes && tripRes.success === true) {
          ...
          setTours((prevTours) =>
            prevTours.filter((tour: any) => tour?.id !== id)
          );
        } else {
    ...
    <div className="text-gray-950 overflow-hidden h-screen w-full p-4 bg-gray-100 ">
      <div className="overflow-y-scroll h-screen w-full pb-16">
        ...
        {tours.map((tour: any) => (
          <div
            key={tour.id}
            className="bg-white py-3 px-4 pr-9 mb-3 rounded-lg shadow-lg flex flex-col justify-center"
          >
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <div className="flex gap-5">
                <Link
                  href={`/trip/${tour.id}`}
                  className="px-3 py-2 rounded-lg text-white bg-teal-500 hover:bg-teal-600 transition duration-150"
                >
                  View
                </Link>
                <Link
                  href={`/dashboard/tours/update-trip/${tour.id}`}
                  className="px-3 py-2 rounded-lg text-white bg-blue-500 hover:bg-blue-600 transition duration-150"
                >
                  Edit
                </Link>
                <button
            ...
            <div className="">
              ...
              >
                {seeMore[tour.id] ? (
                  <span>
                    Hide Images <IoIosArrowUp className="inline-block" />
                  </span>
                ) : (
                  <span>
                    Show Images <IoIosArrowDown className="inline-block" />
                  </span>
                )}
              </button>
              {seeMore[tour.id] && (
